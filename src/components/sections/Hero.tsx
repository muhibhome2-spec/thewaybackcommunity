...
/**
 * Above-the-fold pitch. Single clear primary CTA, with a small trust
 * line directly underneath (no scroll required to find risk reversal).
 * Credentials sit below as a quiet authority strip.
 */
  ...
  return (
    ...
    >
      <Container>
        ...
        <h1 className="animate-rise [animation-delay:100ms] mt-[22px] text-h1 font-light">
          Nobody fights for their
          <br />
          teenager <Em>alone.</Em>
        </h1>
        <p className="animate-rise [animation-delay:180ms] mx-auto mt-[26px] max-w-narrow text-sub font-light text-sub">
        ...
        <p className="animate-rise [animation-delay:300ms] mt-3 font-mono text-[12px] uppercase tracking-[0.1em] text-muted">
        ...
        <ul className="animate-rise [animation-delay:340ms] mt-[34px] flex flex-wrap justify-center gap-x-[26px] gap-y-2.5">
          {heroCredentials.map((c, i) => (
            <li
              key={c}
              className={
                "relative font-mono text-[12px] tracking-[0.1em] uppercase text-muted " +
                (i < heroCredentials.length - 1
                  ? "after:content-[''] after:absolute after:right-[-14px] after:top-1/2 after:w-[3px] after:h-[3px] after:-translate-y-1/2 after:rounded-full after:bg-muted/60"
                  : "")
              }
            >
              {c}
