      ...
      <dl className="mt-11 grid grid-cols-2 lg:grid-cols-4 gap-[18px]">
        {stats.map((s) => (
          <div key={s.label} className="text-center px-3 py-6">
            ...
            <dd className="text-stat font-light text-clay">
              {s.value}
            </dd>
      ...
      <div className="mt-9 grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {testimonials.map((t, i) => (
          <Reveal key={t.author} delay={i * 60}>
            <figure className="h-full bg-soft border border-hairline rounded-card p-7 flex flex-col gap-4 min-h-[200px] transition-[transform,box-shadow] duration-200 hover:-translate-y-1 hover:shadow-card motion-reduce:hover:translate-y-0">
              <Stars rating={t.rating} />
              <blockquote className="text-[18px] font-light leading-snug tracking-[-0.01em] text-ink">
                <p>&ldquo;{t.quote}&rdquo;</p>
              </blockquote>
              <figcaption className="mt-auto font-mono text-[12px] uppercase tracking-[0.1em] text-clay">
                {t.author}
              </figcaption>
            </figure>
          </Reveal>
